...
## 2026-03-02 (增量开发十: 推理与训练流程统一封装)

- **目标**：解决 `app.py` 中过激耦合的底层张量与分词逻辑，实现“计算管线化”。保证日后添加任意新模型架构 (如 Transformer) 均能开箱复用。
- **当前进展**：
  1. **建立 `DualEncoderEngine`**：在单独的 `engine.py` 中封装了面向对象的训练与微调管线。它向外暴露了极简的三个接口：(`.train()`, `.encode()`, `.predict_similarity()`)。
  2. **前端纯净度大幅提升**：移除了 `app.py` 里所有负责把句子分词、转 Tensor、转移至 device (CUDA/MPS)、矩阵对齐等重活。现在界面代码仅需负责最纯粹的输入接收与 Streamlit UI 布局，大幅度提高了代码的可维护性和健壮性。
- **阶段完成**：新塔扩展能力与兼容性已提升至最高标准，整个框架演变为更标准的工业化 MVC (Model-View-Controller) 结构。

## 2026-03-02 (增量开发十一: 解决 Transformer 表示坍塌)

- **目标**：修复 `TransformerDualEncoder` 在从零基础未经预训练时导致的 Loss 异常与余弦打分全为 1.0 的表示层坍塌问题。
- **当前进展**：
  1. **层归一化前置 (Pre-LN)**：在没有大基座预热学习率的情况下，原生 Transformer 默认的 Post-LN 会导致初期梯度丢失或乱跳。现已开启 `norm_first=True`。
...
## 2026-03-07 (增量开发十四: 向量计算缓存落地与 IVF 百万级加速)

- **目标**：将向量检索场景的 Doc 向量缓存落地，避免重复推理，并引入千万级倒排加速。
- **当前进展**：
  1. **架构改造**：在 `app.py` 中实现了基于本地文件 (`.npy` 用于张量，`.pkl` 用于句子文本元数据) 的缓存机制，完美隔绝了不同架构/维度的空间冲突。
  2. **命名规范**：遵循 `doc_cache_{model_type}_{dataset_name}_embed{embed_dim}.npy`，严谨管控各个模型下编码出的不同高维空间特征集。
  3. **前端加速体验**：保留了原有的“构建索引”操作流程，当命中本地盘缓存时，大幅砍掉所有显存运算时间，提示极速恢复。
  4. **FAISS 百万级进阶**：从暴力穷举 `IndexFlatIP` 进化为带有倒排索引的 `IndexIVFFlat`。实现了基于 `kmeans` 的全局聚类预训练 `faiss_index.train()`，并根据数据量动态调整聚类中心(`nlist`)与探测桶(`nprobe`)数量，在保持极高精度的同时，彻底打破了线性检索时间的瓶颈。
- **阶段完成**：在向量检索实战部分去除了高频次、重叠的重复提取开销，全面对齐了工业界 "Query实时，Doc离线" 以及 "基于 IVF 的海量倒排" RAG 标准运作范式。

## 2026-03-07 (增量开发十五: PyTorch DataLoader 喂参补天)

- **目标**：榨干 NVIDIA GPU 极限算力，解决数据 Host-to-Device 传输慢导致的 GPU “空转挨饿” 瓶颈。
- **当前进展**：
  1. **锁页内存映射**：在 `data.py` 中的两种核心数据加载管线中均注入了 `pin_memory=torch.cuda.is_available()`，通过不经 CPU 分页缓存直接做 DMA 显存直传，打通了物理 RAM 到 VRAM 的高速通道。
  2. **多进程异步预取**：剥离单线程的阻塞式加载，为 Linux/Mac 环境拉起 `num_workers=4` 和 `prefetch_factor=2` 的预取队列（并兼顾 Windows 环境下的进程保护），利用 CPU 空余算力提前将下一个 Batch 的增量填入弹夹。

## 2026-03-07 (增量开发十六: 预训练高级微调策略)

- **目标**：在预训练大模型 (RoBERTa 及 LoRA) 的微调阶段引入符合大模型训练标准的动态学习率调度与防过拟合机制。
- **当前进展**：
  1. **线性预热与余弦退火**：在 `engine.py` 的微调分支中剥离了恒定学习率。使用 `SequentialLR` 串联了 `LinearLR` (占据总 Step 的前 10% 进行 Warmup，防止初始大步长破坏预训练底层稳定权重) 和 `CosineAnnealingLR` (在余下 90% 的 Step 中沿着余弦半周期平滑衰减至极小值，辅助模型在极小点收敛)。
  2. **早停机制 (Early Stopping)**：引入了以 Epoch 平均 Loss 为核心的早停断路器计算。当探测到连续 2 个 Epoch (配置的 `patience` 阈值) 的 Loss 均未低于历史最佳记录时，强制提前熔断训练循环，节约大量算力并保护模型不陷入过拟合泥潭。
- **阶段完成**：预训练相关的模型调优正式看齐业界标准微调范式 (Warmup + Decay + Early Stop)。
