...
- [x] 引入全量 LCQMC 数据集 (`lcqmc_max`) 约 24 万对。
- [x] 改进前端 `app.py`，增加数据集规模选择器，向下兼容 10 万、1 万和迷你规模，并动态推荐匹配的超参 (Epoch/LR/BatchSize)。

### 阶段八：向量库进阶 (In Progress)

- [ ] 引入离线序列化缓存机制，实现分词及编码过程的秒级加载，解决 24 万数据加载瓶颈。
- [ ] 建立双重微型向量库对比：基于 PyTorch 矩阵并行的暴力精准检索基线 vs FAISS 近似最近邻检索。
