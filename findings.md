...
1. ~~词量增长下的存储与加载速度优化机制。~~
   **解决方案 (2026-03-01)**：将 `get_dataloader` 的产物（`STSDataset` 和 `tokenizer`）通过 `pickle` 进行离线缓存落盘 (`data/cache`)。采用 Hash(路径+修改时间+分词类型维度) 作为唯一标识，使得在后续运行同样的配置时，跳过数十万行的文件读取、截词断句和转 Tensor 的步骤，将原本需要十几秒到一分钟甚至更长的预处理时间压缩至极速（**~1-3秒内**）。

2. ~~检索全量 24w 数据时的向量库构建方案（暴力计算之外的近似最近邻方案如 FAISS）。~~
   **解决方案 (2026-03-01)**：实施双轨方案并进行耗时对照：
   - 采用 PyTorch 原生矩阵算法 (`torch.matmul`)：构建 $N \times D$ 浮点矩阵（约 38w 合并去重长句），在 GPU/CUDA 加速下仍能达到数十毫秒内的高速运算。
   - 采用 FAISS 近邻引擎：由于其内存索引的强大底座，耗时常常在 1 ms 以下，并且无需任何显卡的昂贵显存资源，进一步确立了在检索领域的工业界绝对核心地位。
...
- 【向量维稳】：要想在 FAISS / PyTorch 中用直接内积（点乘）代替余弦相似度的繁杂算式，则**在进入库之前必须对模型输出的 Embeddings 进行严格的强迫 L2 Normalization (`F.normalize(p=2)`)**。
