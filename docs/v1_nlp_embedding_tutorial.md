...
- **优化与增强**：纯粹的 CNN 容易过拟合且在计算余弦相似度时遭遇高维灾难。因此我们加入了 `BatchNorm1d` 稳定梯度，`Dropout(0.3)` 加 30% 随机失活防止过拟合，以及基于 `Linear` 的投影层将拼接后的 192 维特征压缩回 128 维。
...
- **余弦空间的表示坍塌 (Representation Collapse)**：我们曾试着在最后一层使用 `tanh` 激活函数以匹配余弦函数 [-1, 1] 的特性。灾难发生了：不论什么句子输入，输出永远几乎一样！原来是高学习率将激活值全部推向了 `tanh` 两侧极端（都近似于1或-1），导致特征多样性全盘丢失，直方图上的绿柱（相似）与红柱（不相似）完全合体。移除 `tanh` 恢复**线性投射**后，特征的多向性得以释放，LSTM 以 0.11 的超低 Loss 彻底击穿了另外两种架构。
